...
simdata.g=9.7946;%gravity(simdata.latitude,simdata.altitude);
% Sampling period [s]
simdata.Ts=1/1000;
% Path to the folder where the IMU data file that should be processed is
% located.
simdata.path=maindir;
% Initial heading [rad]
...
simdata.sigma_initial_acc_bias=0.001*ones(3,1);           % Accelerometer biases (x,y,z platform coordinate axis)[m/s^2]
simdata.sigma_initial_gyro_bias=0.001*pi/180*ones(3,1);   % Gyroscope biases (x,y,z platform coordinate axis) [rad/s]                               
